...
dir_img = '/home/zhangkai/zk/data/TMJ_imgs/'
dir_mask = '/home/zhangkai/zk/data/TMJ_masks/'
...
def train_net(net,
    ...
    train = list(dataset)[0:n_train]
    train_loader = DataLoader(train, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=False)
    val_loader = DataLoader(val, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=False)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                mid_imgs = mid_imgs.to(device=device, dtype=torch.float32)
                final_imgs = final_imgs.to(device=device, dtype=torch.float32)
 

                masks_pred = net(imgs,mid_imgs,final_imgs)
                # if (epoch+1) % 20 == 0:
                ...
                loss2 = dice_loss(masks_pred, true_masks)
                # loss = loss1 + loss2
        ...
        if save_cp:
            if (epoch+1) % 10 == 0:
                try:
                    ...
                    pass
                if not os.path.exists(dir_checkpoint + '72Img_res/'):
                    os.mkdir(dir_checkpoint + '72Img_res/')
                torch.save(net.state_dict(),
                           dir_checkpoint + f'72Img_res/' +f'CP_epoch{epoch + 1}.pth')
