...
def image_augmentation(img, label, augnum):  # num means batch_size
    ...
    n = 0
    for batch in image_datagen.flow(img, batch_size=1, save_to_dir=path, save_prefix='aug', save_format='png',
                                    seed=seed):
        n += 1
        if n > augnum:
            break
    ...
    for batch in image_datagen.flow(label, batch_size=1, save_to_dir=path_, save_prefix='aug', save_format='png',
                                    seed=seed):
        n += 1
        if n > augnum:
            break
